...
""")
    ...
    // boolean: true false status == true staus == false
    @Query("""
    SELECT p FROM Products p
    WHERE p.hidden = false
      AND p.category.categoryId = :categoryId
...
""")
    ...
    @Query("""
    SELECT new com.gearsy.gearsy.dto.ProductAdminDTO(
        p.productId, p.name, p.price, p.stock, p.description,
        p.images, p.compatibilitySpecs, p.hidden,
    ...
    WHERE (:keyword IS NULL OR :keyword = '' 
           ...
           OR LOWER(p.description) LIKE LOWER(CONCAT('%', :keyword, '%')))
""")
    Page<ProductAdminDTO> searchProductsWithKeyword(

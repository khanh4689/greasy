...
public class CartServiceImpl implements CartService {
    ...
    public BigDecimal calculateTotal(Users user) {
        // Lấy tất cả cart_items của user
        List<Cart_Items> items = cartItemRepository.findByCart_User(user);

        // Tính tổng: sum(quantity * price)
        return items.stream()
    ...
    @Override
    public int countItems(Users user) {
        return cartItemRepository.sumQuantityByUser(user).intValue();
    }
